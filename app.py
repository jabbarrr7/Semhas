...
TIMEBLOCKS_JUMAT = {
    ...
    3: ["08:00-10:30", "09:40-12:10",
        "14:00-16:30"]  # Friday excludes midday block to respect 12:10 limit
}
...
def schedule_sections_with_ortools(max_seconds=60, semester_type='semua'):
    ...
    for course_id, course_sections in sections_by_course.items():
        # Get course and its selected_by list
        # Try lookup by ID first, fallback to name
        course = None
        try:
        ...
        # Fallback: lookup by course name (case-insensitive)
        if not course and course_sections:
            ...
            if course_name:
                ...
                if course:
                    print(f"[DEBUG] Found course by name fallback: {course_name}")
        ...
        # Distribute sections to lecturers by current load, respecting max_load when possible
        ...
        # Assign one by one: pick lecturer with lowest current SKS who stays within max_load if possible
        for sec in course_sections:
            # Build candidate list with (will_exceed, current_sks, lecturer)
            candidates = []
            for lect in selected_by:
                cur = int(lecturer_current_sks.get(lect, 0))
                max_cap = int(lecturer_max_sks.get(lect, 12) or 12)
                will_exceed = 1 if (cur + sks_course) > max_cap else 0
                candidates.append((will_exceed, cur, lect))
            # Prefer not exceeding, then lowest current_sks
            candidates.sort(key=lambda t: (t[0], t[1]))
            chosen = candidates[0][2] if candidates else (selected_by[0] if selected_by else None)
            if not chosen:
                continue
            # Persist assignment
